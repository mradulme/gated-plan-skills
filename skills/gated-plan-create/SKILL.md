...
4. **Split into commit-sized items.** This is the core skill. Each item is:
   ...
   **Optional sounding board (advisory only).** When a phasing or splitting call in steps 3–4 is
   ...
   - `node /Users/mradul/git/gated-plan-skills/skills/_shared/pool.mjs route --role advisory --file .gated-plan-tmp/gpc-q.txt --out .gated-plan-tmp/gpc-sounding.txt` → prints `{chosen:{id,command}, fallbackIds:[...]}`.
